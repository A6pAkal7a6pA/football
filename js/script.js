...
function setUpConfiguration() {
  let firstChildRoadItem = document.querySelector(
    ".road-top .road__item:first-child"
  );
  ...
  document.querySelector(".road-bot").style.marginLeft =
    marginMidLine + 10 + "px";
  let checkInner = document.querySelector(".check__inner");
  document.querySelector(".road__content").style.marginLeft =
    checkInner.getBoundingClientRect().left + "px";
  midLine.style.width =
    document.documentElement.clientWidth -
    checkInner.getBoundingClientRect().left -
    marginMidLine +
    "px";
}
setUpConfiguration();
...
function ballPosition() {
  document.documentElement.style.setProperty(
    "--ball-desktop",
    posContainer.getBoundingClientRect().left - 35 + "px"
  );
  document.documentElement.style.setProperty(
    "--ball-mob",
    posContainer.getBoundingClientRect().width / 3 + "px"
  );
}
ballPosition();
...
window.addEventListener("scroll", () => {
  if (
    posContainer.getBoundingClientRect().y <= 0 &&
    posContainer.getBoundingClientRect().y >=
      -posContainer.getBoundingClientRect().height
  ) {
    ...
    document
      .querySelector(".secret-ball__text")
      .parentElement.classList.remove("active");
  }
});

let secretBall = document.querySelector(".secret-ball");
secretBall.addEventListener("click", () =>
  secretBall.classList.toggle("active")
);
  ...
  document.querySelectorAll(".collections-road__info-image").forEach((item) =>
    item.addEventListener("click", () => {
      ...
      if (popup.classList.contains("active")) {
        ...
        Array.from(
          document.querySelectorAll(".collections-road__info-popup")
...
document
  .querySelector(".main-presale__exit")
